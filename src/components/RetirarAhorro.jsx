...
import { Search, ArrowDownCircle, Save, PiggyBank, AlertTriangle } from 'lucide-react';

const MOTIVOS = ['Retiro parcial', 'Retiro total', 'Cancelacion de cuenta', 'Gastos personales', 'Otro'];

export default function RetirarAhorro() {
  const [busqueda, setBusqueda]   = useState('');
  const [cuentaSel, setCuentaSel] = useState(null);
  const [form, setForm]           = useState({ monto: '', motivo: '', referencia: '', fecha: '', observaciones: '' });
  ...
  const handleSave = () => {
    if (!cuentaSel || !form.monto || !form.motivo) return;
    if (cuentaSel.saldo !== undefined && parseFloat(form.monto) > cuentaSel.saldo) {
      setMontoError(true); return;
    }
    setMontoError(false);
    setSaved(true);
    setTimeout(() => { setSaved(false); setForm({ monto: '', motivo: '', referencia: '', fecha: '', observaciones: '' }); }, 2500);
  };
    ...
    <div style={{ maxWidth: '820px', margin: '0 auto' }}>
      ...
      {saved && (
        <div style={{ background: '#dcfce7', border: '1px solid #86efac', borderRadius: '10px', padding: '12px 18px', marginBottom: '20px', color: '#166534', fontSize: '13px', fontWeight: '600', display: 'flex', alignItems: 'center', gap: '8px' }}>
          <Save size={15} /> Retiro registrado correctamente.
        </div>
      )}
      ...
      <div style={{ background: '#fff', borderRadius: '16px', border: '1px solid #e4ecf5', boxShadow: '0 1px 4px rgba(0,0,0,0.05)', marginBottom: '20px', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '22px 24px' }}>
          ...
          {filtradas.map(c => (
            <div key={c.id} onClick={() => { setCuentaSel(c); setMontoError(false); }} style={{ padding: '14px 16px', borderRadius: '10px', border: `1.5px solid ${cuentaSel?.id === c.id ? '#dc2626' : '#e2e8f0'}`, marginBottom: '8px', cursor: 'pointer', background: cuentaSel?.id === c.id ? '#fff5f5' : '#fafbfd', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
              ...
              <div style={{ textAlign: 'right' }}>
                ...
                <div style={{ fontSize: '11px', color: '#94a3b8' }}>Saldo disponible</div>
              </div>
            </div>
          ))}
        </div>
      ...
      {cuentaSel && (
        <div style={{ background: '#fff', borderRadius: '16px', border: '1px solid #e4ecf5', boxShadow: '0 1px 4px rgba(0,0,0,0.05)', overflow: 'hidden' }}>
          <div style={{ padding: '16px 24px', borderBottom: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', gap: '10px' }}>
            <ArrowDownCircle size={17} color="#dc2626" />
            <span style={{ fontFamily: 'Playfair Display, serif', fontSize: '16px', fontWeight: '700', color: '#040e2e' }}>Datos del retiro</span>
            <span style={{ marginLeft: 'auto', fontSize: '12px', color: '#94a3b8' }}>{cuentaSel.clienteNombre}</span>
          </div>

          {montoError && (
            <div style={{ margin: '16px 24px 0', background: '#fee2e2', border: '1px solid #fca5a5', borderRadius: '10px', padding: '11px 16px', color: '#991b1b', fontSize: '13px', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <AlertTriangle size={15} /> El monto excede el saldo disponible.
            </div>
          )}

          <div style={{ padding: '22px 24px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '16px' }}>
            <div>
              {lbl('Monto del retiro *')}
              <input type="number" value={form.monto} onChange={e => { setForm(p => ({ ...p, monto: e.target.value })); setMontoError(false); }} placeholder="0.00" style={{ ...inp, borderColor: montoError ? '#ef4444' : '#e2e8f0' }} />
            </div>
            <div>
              {lbl('Motivo *')}
              <select value={form.motivo} onChange={e => setForm(p => ({ ...p, motivo: e.target.value }))} style={{ ...inp, cursor: 'pointer' }}>
          ...
          <div style={{ padding: '0 24px 22px', display: 'flex', justifyContent: 'flex-end' }}>
            <button onClick={handleSave} style={{ background: '#dc2626', border: 'none', borderRadius: '10px', padding: '11px 28px', fontSize: '13px', fontWeight: '600', color: '#fff', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif', display: 'flex', alignItems: 'center', gap: '7px' }}>
              <ArrowDownCircle size={14} /> Registrar retiro
            </button>
          </div>
        </div>
      )}
      ...
      {!cuentaSel && !busqueda && (
        <div style={{ background: '#fff', borderRadius: '16px', border: '1px solid #e4ecf5', padding: '48px', textAlign: 'center', color: '#94a3b8', boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>
          ...
          <p style={{ fontSize: '14px' }}>Busca una cuenta de ahorro para registrar el retiro</p>
        </div>
      )}
    </div>
  );
