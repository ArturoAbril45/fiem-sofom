...
import { Search, PiggyBank, Save, TrendingDown, CheckCircle, AlertCircle, Loader } from 'lucide-react';
...
export default function RetirarAhorro() {
  ...
  const handleSave = async () => {
    ...
    if (Number(form.monto) > cuentaSel.saldo) {
      setEstado('error'); setMensaje(`Saldo insuficiente. Saldo disponible: ${fmt(cuentaSel.saldo)}`);
      setTimeout(() => setEstado(null), 4000); return;
    }
    setEstado('loading');
    try {
      ...
      setEstado('ok');
      setMensaje(`Retiro registrado. Nuevo saldo: ${fmt(data.nuevoSaldo)}`);
    ...
    <div style={{ maxWidth: '820px', margin: '0 auto' }}>
          ...
          {filtradas.map(c => (
            <div key={c._id} onClick={() => setCuentaSel(c)} style={{ padding: '14px 16px', borderRadius: '10px', border: `1.5px solid ${cuentaSel?._id === c._id ? '#0e50a0' : '#dceaf8'}`, marginBottom: '8px', cursor: 'pointer', background: cuentaSel?._id === c._id ? '#eff6ff' : '#fafcff', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <div style={{ textAlign: 'right' }}>
                ...
                <div style={{ fontSize: '11px', color: '#90aac8' }}>Saldo disponible</div>
              </div>
            </div>
          ))}
        </div>
      ...
      {cuentaSel && (
        <div style={{ background: '#fff', borderRadius: '16px', border: '1px solid #dceaf8', boxShadow: '0 2px 12px rgba(14,80,160,0.05)', overflow: 'hidden' }}>
          <div style={{ padding: '16px 24px', borderBottom: '1px solid #f0f6ff', display: 'flex', alignItems: 'center', gap: '10px' }}>
            <div style={{ width: '32px', height: '32px', background: '#fee2e2', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><TrendingDown size={16} color="#dc2626" /></div>
            <span style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '17px', fontWeight: '700', color: '#0a2d5e' }}>Retiro</span>
            <span style={{ marginLeft: 'auto', fontSize: '12px', color: '#90aac8' }}>{cuentaSel.clienteNombre} · Saldo: {fmt(cuentaSel.saldo)}</span>
          </div>
          <div style={{ padding: '22px 24px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '16px' }}>
            <div>{lbl('Monto del retiro *')}<input type="number" value={form.monto} onChange={e => setForm(p => ({ ...p, monto: e.target.value }))} placeholder="0.00" style={inp} /></div>
          ...
          <div style={{ padding: '0 24px 22px', display: 'flex', justifyContent: 'flex-end' }}>
            <button onClick={handleSave} disabled={estado === 'loading'} style={{ background: estado === 'loading' ? '#90aac8' : '#dc2626', border: 'none', borderRadius: '10px', padding: '11px 28px', fontSize: '13px', fontWeight: '600', color: '#fff', cursor: estado === 'loading' ? 'not-allowed' : 'pointer', fontFamily: 'DM Sans, sans-serif', display: 'flex', alignItems: 'center', gap: '7px', boxShadow: '0 4px 14px rgba(220,38,38,0.28)' }}>
              {estado === 'loading' ? <><Loader size={14} style={{ animation: 'spin 1s linear infinite' }} /> Procesando...</> : <><Save size={14} /> Registrar retiro</>}
            </button>
          </div>
        </div>
      )}
      ...
      {!cuentaSel && !busqueda && (
        <div style={{ background: '#fff', borderRadius: '16px', border: '1px solid #dceaf8', padding: '48px', textAlign: 'center', color: '#90aac8', boxShadow: '0 2px 12px rgba(14,80,160,0.05)' }}>
          ...
          <p style={{ fontSize: '14px' }}>Busca una cuenta activa para registrar el retiro</p>
        </div>
      )}
