...
export default function ConsultarCredito() {
  ...
  const badge = (e) => {
    const map = {
      Vigente:   ['#dcfce7', '#166534'],
      Vencido:   ['#fee2e2', '#991b1b'],
      Liquidado: ['#f1f5f9', '#475569'],
    };
    ...
    <div style={{ maxWidth: '960px', margin: '0 auto' }}>
      ...
      <div style={{ background: '#fff', borderRadius: '16px', border: '1px solid #e4ecf5', boxShadow: '0 1px 4px rgba(0,0,0,0.05)', overflow: 'hidden' }}>
        <div style={{ padding: '16px 24px', borderBottom: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', gap: '10px' }}>
          <CreditCard size={17} color="#1565c0" />
          <span style={{ fontFamily: 'Playfair Display, serif', fontSize: '16px', fontWeight: '700', color: '#040e2e' }}>Creditos</span>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: '#f8fafc' }}>
                {['Folio', 'Cliente', 'Producto', 'Monto', 'Saldo', 'Prox. pago', 'Estatus', ''].map(h => (
            ...
            <tbody>
              {filtrados.length === 0 ? (
                ...
                <tr key={c.id} style={{ borderTop: '1px solid #f1f5f9', background: i % 2 === 0 ? '#fff' : '#fafbfd' }}>
                  <td style={{ padding: '12px 16px', fontSize: '12px', fontFamily: 'monospace', color: '#475569' }}>{c.folio}</td>
                  ...
                  <td style={{ padding: '12px 16px', fontSize: '13px', color: '#475569' }}>${c.monto?.toLocaleString('es-MX')}</td>
                  <td style={{ padding: '12px 16px', fontSize: '13px', fontWeight: '600', color: '#dc2626' }}>${c.saldo?.toLocaleString('es-MX')}</td>
                  <td style={{ padding: '12px 16px', fontSize: '13px', color: '#475569' }}>{c.proxPago}</td>
                  <td style={{ padding: '12px 16px' }}><span style={badge(c.estatus)}>{c.estatus}</span></td>
                  <td style={{ padding: '12px 16px' }}>
                    <button
                      onClick={() => setSeleccionado(c)}
                      style={{ background: '#eef2f7', border: 'none', borderRadius: '7px', padding: '7px 12px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '5px', fontSize: '12px', fontWeight: '600', color: '#1565c0', fontFamily: 'DM Sans, sans-serif' }}
                    >
                      <Eye size={13} /> Ver
                    </button>
                  </td>
      ...
      {seleccionado && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(4,14,46,0.5)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: '#fff', borderRadius: '20px', width: '100%', maxWidth: '520px', boxShadow: '0 24px 80px rgba(13,31,92,0.18)', overflow: 'hidden' }}>
            <div style={{ background: '#fff', borderBottom: '1px solid #e4ecf5', padding: '22px 28px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: '12px', color: '#94a3b8', marginTop: '2px' }}>Folio: {seleccionado.folio}</div>
                </div>
              </div>
              <button
            ...
            <div style={{ padding: '24px 28px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
              {[
                ...
                { label: 'Prox. pago',   val: seleccionado.proxPago },
