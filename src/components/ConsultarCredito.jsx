...
import { Search, CreditCard, Eye, X, RefreshCw, AlertCircle, CheckCircle, Loader } from 'lucide-react';
...
export default function ConsultarCredito() {
  ...
  const cambiarEstatus = async (id, estatus) => {
    try {
      await fetch(`${API}/api/creditos/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ estatus }),
      });
      setNotif({ tipo: 'ok', msg: `Estatus actualizado a ${estatus}.` });
      setTimeout(() => setNotif(null), 3000);
      setSeleccionado(null);
      cargar();
    } catch {
      setNotif({ tipo: 'error', msg: 'Error al actualizar estatus.' });
      setTimeout(() => setNotif(null), 3000);
    }
  };
  ...
  const badge = (e) => {
    const map = { Vigente: ['#dcfce7','#166534'], Vencido: ['#fee2e2','#991b1b'], Liquidado: ['#f1f5f9','#475569'] };
    ...
    <div style={{ maxWidth: '960px', margin: '0 auto' }}>
      ...
      <div style={{ background: '#fff', borderRadius: '16px', border: '1px solid #dceaf8', boxShadow: '0 2px 12px rgba(14,80,160,0.05)', overflow: 'hidden' }}>
        <div style={{ padding: '16px 24px', borderBottom: '1px solid #f0f6ff', display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <span style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '17px', fontWeight: '700', color: '#0a2d5e' }}>Creditos</span>
        ...
        <div style={{ overflowX: 'auto' }}>
          {cargando ? (
            <div style={{ padding: '60px', textAlign: 'center', color: '#90aac8', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px' }}>
              <Loader size={18} style={{ animation: 'spin 1s linear infinite' }} /> Cargando creditos...
            </div>
          ) : (
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: '#f4f8fd' }}>
                  {['Folio', 'Cliente', 'Producto', 'Monto', 'Saldo', 'Inicio', 'Estatus', ''].map(h => (
              ...
              <tbody>
                {filtrados.length === 0 ? (
                  <tr><td colSpan={8} style={{ padding: '48px', textAlign: 'center', color: '#90aac8', fontSize: '13px' }}>No se encontraron creditos</td></tr>
                ) : filtrados.map((c, i) => (
                  <tr key={c._id} style={{ borderTop: '1px solid #f0f6ff', background: i % 2 === 0 ? '#fff' : '#fafcff' }}>
                    ...
                    <td style={{ padding: '12px 16px', fontSize: '13px', color: '#4a6a94' }}>{fmt(c.monto)}</td>
                    <td style={{ padding: '12px 16px', fontSize: '13px', fontWeight: '700', color: '#dc2626' }}>{fmt(c.saldo)}</td>
                    <td style={{ padding: '12px 16px', fontSize: '13px', color: '#4a6a94' }}>{c.fechaInicio ? new Date(c.fechaInicio).toLocaleDateString('es-MX') : '—'}</td>
          ...
          <div style={{ background: '#fff', borderRadius: '20px', width: '100%', maxWidth: '520px', boxShadow: '0 24px 80px rgba(10,45,94,0.2)', overflow: 'hidden' }}>
            <div style={{ background: '#fff', borderBottom: '1px solid #dceaf8', padding: '22px 28px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <div style={{ width: '40px', height: '40px', background: '#e8f2fc', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><CreditCard size={20} color="#0e50a0" /></div>
                <div>
            ...
            <div style={{ padding: '24px 28px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
              {[
                ...
                { label: 'Saldo actual',  val: fmt(seleccionado.saldo) },
                { label: 'Plazo',         val: `${seleccionado.plazo} periodos` },
                { label: 'Frecuencia',    val: seleccionado.frecuencia },
                { label: 'Tasa interes',  val: `${seleccionado.tasaInteres || 0}%` },
                { label: 'Fecha inicio',  val: seleccionado.fechaInicio ? new Date(seleccionado.fechaInicio).toLocaleDateString('es-MX') : '—' },
            ...
            <div style={{ padding: '0 28px 24px', display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
              {seleccionado.estatus === 'Vigente' && (
                <button onClick={() => cambiarEstatus(seleccionado._id, 'Vencido')} style={{ flex: 1, padding: '10px', border: '1.5px solid #fca5a5', background: '#fff', borderRadius: '10px', fontSize: '12px', fontWeight: '600', color: '#dc2626', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>Marcar vencido</button>
              )}
              {seleccionado.estatus !== 'Liquidado' && (
                <button onClick={() => cambiarEstatus(seleccionado._id, 'Liquidado')} style={{ flex: 1, padding: '10px', border: 'none', background: '#166534', borderRadius: '10px', fontSize: '12px', fontWeight: '600', color: '#fff', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>Marcar liquidado</button>
              )}
