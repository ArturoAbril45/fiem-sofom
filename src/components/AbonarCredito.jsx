...
import { Search, DollarSign, Save, CreditCard } from 'lucide-react';
...
export default function AbonarCredito() {
  ...
  const handleSave = () => {
    if (!creditoSel || !form.monto || !form.metodo) return;
    setSaved(true);
    ...
    <div style={{ maxWidth: '820px', margin: '0 auto' }}>
      ...
      {saved && (
        <div style={{ background: '#dcfce7', border: '1px solid #86efac', borderRadius: '10px', padding: '12px 18px', marginBottom: '20px', color: '#166534', fontSize: '13px', fontWeight: '600', display: 'flex', alignItems: 'center', gap: '8px' }}>
          <Save size={15} /> Abono registrado correctamente.
        </div>
      )}
      ...
      <div style={{ background: '#fff', borderRadius: '16px', border: '1px solid #e4ecf5', boxShadow: '0 1px 4px rgba(0,0,0,0.05)', marginBottom: '20px', overflow: 'hidden' }}>
        <div style={{ padding: '16px 24px', borderBottom: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', gap: '10px' }}>
          <Search size={17} color="#1565c0" />
          <span style={{ fontFamily: 'Playfair Display, serif', fontSize: '16px', fontWeight: '700', color: '#040e2e' }}>Buscar credito</span>
        </div>
        <div style={{ padding: '22px 24px' }}>
          ...
          {filtrados.length === 0 && busqueda && (
            <p style={{ color: '#94a3b8', fontSize: '13px', textAlign: 'center', padding: '12px 0' }}>No se encontraron creditos</p>
          )}
          {filtrados.map(c => (
            <div key={c.id} onClick={() => setCreditoSel(c)} style={{ padding: '14px 16px', borderRadius: '10px', border: `1.5px solid ${creditoSel?.id === c.id ? '#1565c0' : '#e2e8f0'}`, marginBottom: '8px', cursor: 'pointer', background: creditoSel?.id === c.id ? '#eff6ff' : '#fafbfd', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <div style={{ fontSize: '13px', fontWeight: '600', color: '#0d1f5c' }}>{c.clienteNombre}</div>
                <div style={{ fontSize: '12px', color: '#94a3b8', marginTop: '2px' }}>Folio: {c.folio} · {c.producto}</div>
              </div>
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontSize: '13px', fontWeight: '700', color: '#dc2626' }}>${c.saldo?.toLocaleString('es-MX')}</div>
      ...
      {creditoSel && (
        <div style={{ background: '#fff', borderRadius: '16px', border: '1px solid #e4ecf5', boxShadow: '0 1px 4px rgba(0,0,0,0.05)', overflow: 'hidden' }}>
          <div style={{ padding: '16px 24px', borderBottom: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <span style={{ fontFamily: 'Playfair Display, serif', fontSize: '16px', fontWeight: '700', color: '#040e2e' }}>Datos del abono</span>
            <span style={{ marginLeft: 'auto', fontSize: '12px', color: '#94a3b8' }}>{creditoSel.clienteNombre}</span>
          </div>
          <div style={{ padding: '22px 24px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '16px' }}>
            <div>
              {lbl('Monto del abono *')}
              <input type="number" value={form.monto} onChange={e => setForm(p => ({ ...p, monto: e.target.value }))} placeholder="0.00" style={{ ...inp, borderColor: !form.monto && saved ? '#ef4444' : '#e2e8f0' }} />
            </div>
            <div>
              {lbl('Metodo de pago *')}
      ...
      {!creditoSel && !busqueda && (
        <div style={{ background: '#fff', borderRadius: '16px', border: '1px solid #e4ecf5', padding: '48px', textAlign: 'center', color: '#94a3b8', boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>
          ...
          <p style={{ fontSize: '14px' }}>Busca un credito para registrar el abono</p>
        </div>
      )}
    </div>
  );
