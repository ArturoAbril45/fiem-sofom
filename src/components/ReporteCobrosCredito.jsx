...
export default function ReporteCobrosCredito() {
  const [filtros, setFiltros] = useState({ fechaInicio: '', fechaFin: '', ejecutivo: '', metodo: '', producto: '' });
  ...
  return (
    <div style={{ maxWidth: '980px', margin: '0 auto' }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '20px' }}>
        {[{ l: 'Total cobros', v: '0' }, { l: 'Monto cobrado', v: '$0' }, { l: 'Cobros efectivo', v: '$0' }, { l: 'Cobros transferencia', v: '$0' }].map(({ l, v }) => (
      ...
      <div style={{ background: '#fff', borderRadius: '16px', border: '1px solid #dceaf8', padding: '20px 24px', marginBottom: '20px', boxShadow: '0 2px 12px rgba(14,80,160,0.05)' }}>
        <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', alignItems: 'flex-end' }}>
          ...
          <div style={{ minWidth: '150px' }}>{lbl('Metodo')}<select value={filtros.metodo} onChange={e => f('metodo', e.target.value)} style={{ ...inp, cursor: 'pointer' }}><option value="">Todos</option><option>Efectivo</option><option>Transferencia</option><option>Deposito bancario</option><option>Cheque</option></select></div>
          <div style={{ flex: 1, minWidth: '150px' }}>{lbl('Ejecutivo')}<input value={filtros.ejecutivo} onChange={e => f('ejecutivo', e.target.value)} placeholder="Nombre" style={inp} /></div>
      ...
      <div style={{ background: '#fff', borderRadius: '16px', border: '1px solid #dceaf8', boxShadow: '0 2px 12px rgba(14,80,160,0.05)', overflow: 'hidden' }}>
        <div style={{ padding: '16px 24px', borderBottom: '1px solid #f0f6ff', display: 'flex', alignItems: 'center', gap: '10px' }}>
          <div style={{ width: '32px', height: '32px', background: '#e8f2fc', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Wallet size={16} color="#0e50a0" /></div>
          <span style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '17px', fontWeight: '700', color: '#0a2d5e' }}>Cobros de credito</span>
          <span style={{ marginLeft: 'auto', fontSize: '12px', color: '#90aac8' }}>0 registros</span>
        </div>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead><tr style={{ background: '#f4f8fd' }}>{['Fecha', 'Folio', 'Cliente', 'Producto', 'Monto abono', 'Capital', 'Interes', 'Metodo', 'Ejecutivo'].map(h => <th key={h} style={{ padding: '11px 14px', textAlign: 'left', fontSize: '11px', fontWeight: '700', color: '#90aac8', textTransform: 'uppercase', letterSpacing: '0.06em', whiteSpace: 'nowrap' }}>{h}</th>)}</tr></thead>
          <tbody><tr><td colSpan={9} style={{ padding: '48px', textAlign: 'center', color: '#90aac8', fontSize: '13px' }}><FileText size={28} color="#dceaf8" style={{ display: 'block', margin: '0 auto 12px' }} />Sin cobros registrados.</td></tr></tbody>
        </table>
      </div>
    </div>
  );
