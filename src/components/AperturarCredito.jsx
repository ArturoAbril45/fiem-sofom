...
import { CreditCard, User, Save, RotateCcw, Calendar, CheckCircle, AlertCircle, Loader, Search } from 'lucide-react';
...
const INITIAL = { clienteId: '', clienteNombre: '', clienteCurp: '', producto: '', monto: '', plazo: '', frecuencia: '', tasaInteres: '', comisionApertura: '', fechaInicio: '', ejecutivo: '', ruta: '' };
...
export default function AperturarCredito() {
  ...
  const validate = () => {
    const req = ['clienteNombre', 'producto', 'monto', 'plazo', 'frecuencia', 'fechaInicio'];
    const errs = {};
    ...
    try {
      const res = await fetch(`${API}/api/creditos`, {
        ...
        body: JSON.stringify({
          ...
          comisionApertura: Number(form.comisionApertura) || 0,
      ...
      setFolio(data.folio || '');
      setEstado('ok');
      setMensaje(`Credito aperturado correctamente. Folio: ${data.folio || ''}`);
  ...
  return (
    <div style={{ maxWidth: '820px', margin: '0 auto' }}>
      {estado === 'ok' && (
        <div style={{ background: '#dcfce7', border: '1px solid #86efac', borderRadius: '12px', padding: '16px 20px', marginBottom: '18px', color: '#166534', fontSize: '13px', fontWeight: '600', display: 'flex', alignItems: 'center', gap: '10px' }}>
          <CheckCircle size={16} /> {mensaje}
          {folio && <span style={{ marginLeft: 'auto', background: '#166534', color: '#fff', padding: '4px 12px', borderRadius: '20px', fontSize: '12px' }}>{folio}</span>}
        </div>
      )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
          <div style={{ position: 'relative' }}>
            ...
            <div style={{ position: 'relative' }}>
              ...
              <input value={busCliente} onChange={e => { setBusCliente(e.target.value); setForm(p => ({ ...p, clienteNombre: e.target.value })); setShowDrop(true); }} onFocus={() => setShowDrop(true)} placeholder="Buscar por nombre o CURP..." onBlur={cerrarDrop} style={{ ...inp(errors.clienteNombre), paddingLeft: '36px' }} />
            </div>
          ...
          <div>
            {lbl('CURP')}
            <input value={form.clienteCurp} readOnly style={{ ...inp(false), background: '#f4f8fd', color: '#90aac8' }} />
          </div>
        </div>
      , true)}
      ...
      <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
        ...
        <button onClick={handleSave} disabled={estado === 'loading'} style={{ background: estado === 'loading' ? '#90aac8' : '#0e50a0', border: 'none', borderRadius: '10px', padding: '11px 28px', fontSize: '13px', fontWeight: '600', color: '#fff', cursor: estado === 'loading' ? 'not-allowed' : 'pointer', fontFamily: 'DM Sans, sans-serif', display: 'flex', alignItems: 'center', gap: '7px', boxShadow: '0 4px 14px rgba(14,80,160,0.28)' }}>
          {estado === 'loading' ? <><Loader size={14} style={{ animation: 'spin 1s linear infinite' }} /> Aperturando...</> : <><Save size={14} /> Aperturar credito</>}
        </button>
      </div>
