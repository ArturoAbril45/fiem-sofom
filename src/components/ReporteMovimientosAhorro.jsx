...
import { PiggyBank, Download, FileText } from 'lucide-react';

export default function ReporteMovimientosAhorro() {
  const [filtros, setFiltros] = useState({ fechaInicio: '', fechaFin: '', tipo: '', producto: '', cuenta: '' });
  ...
  return (
    <div style={{ maxWidth: '980px', margin: '0 auto' }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '20px' }}>
        {[{ l: 'Total movimientos', v: '0' }, { l: 'Total depositos', v: '$0' }, { l: 'Total retiros', v: '$0' }, { l: 'Saldo total ahorro', v: '$0' }].map(({ l, v }) => (
      ...
      <div style={{ background: '#fff', borderRadius: '16px', border: '1px solid #dceaf8', padding: '20px 24px', marginBottom: '20px', boxShadow: '0 2px 12px rgba(14,80,160,0.05)' }}>
        <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', alignItems: 'flex-end' }}>
          ...
          <div style={{ minWidth: '130px' }}>{lbl('Tipo')}<select value={filtros.tipo} onChange={e => f('tipo', e.target.value)} style={{ ...inp, cursor: 'pointer' }}><option value="">Todos</option><option>Deposito</option><option>Retiro</option></select></div>
          <div style={{ minWidth: '160px' }}>{lbl('Producto')}<select value={filtros.producto} onChange={e => f('producto', e.target.value)} style={{ ...inp, cursor: 'pointer' }}><option value="">Todos</option>{['AHORRO ORDINARIO','AHORRO CRECEMAX','Inversion 60','Ahorro creceMax 120','AHORRO NOMINA'].map(p => <option key={p}>{p}</option>)}</select></div>
          <div style={{ flex: 1, minWidth: '140px' }}>{lbl('No. cuenta')}<input value={filtros.cuenta} onChange={e => f('cuenta', e.target.value)} placeholder="Numero de cuenta" style={inp} /></div>
          <button style={{ background: '#0891b2', border: 'none', borderRadius: '10px', padding: '11px 20px', fontSize: '13px', fontWeight: '600', color: '#fff', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif', display: 'flex', alignItems: 'center', gap: '6px', boxShadow: '0 4px 14px rgba(8,145,178,0.28)' }}><Download size={14} /> Exportar</button>
        </div>
      </div>
      <div style={{ background: '#fff', borderRadius: '16px', border: '1px solid #dceaf8', boxShadow: '0 2px 12px rgba(14,80,160,0.05)', overflow: 'hidden' }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead><tr style={{ background: '#f4f8fd' }}>{['Fecha', 'No. cuenta', 'Cliente', 'Producto', 'Tipo', 'Monto', 'Saldo anterior', 'Saldo nuevo', 'Metodo'].map(h => <th key={h} style={{ padding: '11px 14px', textAlign: 'left', fontSize: '11px', fontWeight: '700', color: '#90aac8', textTransform: 'uppercase', letterSpacing: '0.06em', whiteSpace: 'nowrap' }}>{h}</th>)}</tr></thead>
          <tbody><tr><td colSpan={9} style={{ padding: '48px', textAlign: 'center', color: '#90aac8', fontSize: '13px' }}><FileText size={28} color="#dceaf8" style={{ display: 'block', margin: '0 auto 12px' }} />Sin movimientos de ahorro registrados.</td></tr></tbody>
        </table>
      </div>
    </div>
  );
