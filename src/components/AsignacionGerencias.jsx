...
import { Building2, User, Save, RotateCcw, Check } from 'lucide-react';

const GERENCIAS = ['MATRIZ (01)', 'Zumpango (02)', 'TIZAYUCA (03)'];

const INITIAL = {
  gerencia: '', gerenteNombre: '', gerenteCurp: '',
  gerenteCelular: '', fechaAsignacion: '', observaciones: '',
};

export default function AsignacionGerencias() {
  ...
  const validate = () => {
    const req = ['gerencia', 'gerenteNombre', 'gerenteCurp', 'fechaAsignacion'];
    const errs = {};
    ...
    <div style={{ maxWidth: '820px', margin: '0 auto' }}>
      {saved && (
        <div style={{ background: '#dcfce7', border: '1px solid #86efac', borderRadius: '10px', padding: '12px 18px', marginBottom: '20px', color: '#166534', fontSize: '13px', fontWeight: '600', display: 'flex', alignItems: 'center', gap: '8px' }}>
          <Check size={15} /> Gerente asignado correctamente.
        </div>
      )}

      {card('Seleccionar gerencia', Building2,
      ...
      {card('Datos del gerente', User,
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px' }}>
          <div>
            {lbl('Nombre completo', true)}
            <input value={form.gerenteNombre} onChange={e => change('gerenteNombre', e.target.value)} placeholder="Nombre del gerente" style={inp(errors.gerenteNombre)} />
            {errors.gerenteNombre && <span style={{ color: '#ef4444', fontSize: '11px' }}>Requerido</span>}
          </div>
          <div>
            {lbl('CURP', true)}
            <input value={form.gerenteCurp} onChange={e => change('gerenteCurp', e.target.value)} placeholder="CURP" style={inp(errors.gerenteCurp)} />
            {errors.gerenteCurp && <span style={{ color: '#ef4444', fontSize: '11px' }}>Requerido</span>}
          </div>
          <div>
            {lbl('Celular')}
            <input type="tel" value={form.gerenteCelular} onChange={e => change('gerenteCelular', e.target.value)} placeholder="10 digitos" style={inp(false)} />
          </div>
